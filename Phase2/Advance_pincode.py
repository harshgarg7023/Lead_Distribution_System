...
# ---------------- CONFIG ----------------
LEADS_FILE = "leads__Probus _ carInfo.xlsx"
...
def pincode_match_level(lead_pin, posp_pin):
    ...
    level: '6', '5', '3', or 'none'
    prefix_len: 6, 5, 3, or 0 (not used in scoring directly, but useful if needed)
    """
...
def match_leads_with_posp(leads, posps):
    ...
    for _, lead in leads.iterrows():
        ...
        # 🔹 Try multiple possible pincode column names for lead
        lead_pincode = (
            lead.get("pincode")
            or lead.get("regpincode")
            or lead.get("lead_pincode")
            or ""
        )
        ...
        for _, posp in posps.iterrows():
            ...
            # 🔹 Try multiple possible pincode column names for POSP
            posp_pincode = (
                posp.get("pincode")
                or posp.get("posp_pincode")
                or posp.get("pin_code")
                or ""
            )
