...
MAX_MATCHES_PER_LEAD = 1   # set to 1 if you want only 1 POSP per lead
# ----------------------------------------
...
def match_leads_with_posp(leads, posps):
        ...
        for _, posp in posps.iterrows():
            ...
            if lead_city_norm and posp_city_norm and lead_city_norm == posp_city_norm:
                ...
                exact_posp_scores.append({
                    ...
                    "last_biz_date": last_biz_date,  # 👈 needed for tie-breaker
                })
            else:
    ...
    # last_biz_date is only for internal tie-break; optional to drop from final output:
    # df = df.drop(columns=["last_biz_date"])
